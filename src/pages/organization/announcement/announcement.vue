    ...
    <v-data-table hide-actions :headers="headers" :items="desserts" :no-data-text="$t('message.no_message')"
      ...
      <template slot="items" slot-scope="props">
        ...
        <td class="text-xs-center">
          <v-tooltip top>
            <v-btn :loading="replayLoading" flat icon color="purple darken-2" slot="activator"
                ...
                <v-layout row wrap>
                  ...
                  <v-flex xs12>
                    ...
                    <v-radio-group v-model="forms.browseRight" :rules="[rules.required]" row>
                      ...
                      <v-radio :label="$t('global.false_public')" color="purple darken-2" value="PRIVATE"></v-radio>
                  ...
                  <v-flex xs12 v-if="forms.browseRight === 'APPOINT'">
                    <v-flex xs12 class="px-0">
                      <v-flex xs12 class="px-0">
                        <v-autocomplete v-model="friends1" :disabled="isUpdating" :items="memberships"
                                        :rules="[rules.required]" :no-data-text="$t('message.no_message')" box chips
                                        color="primary" :label="$t('global.select_memberShips')" item-text="name"
                                        item-value="id" multiple>
                          <template slot="selection" color="purple darken-2" slot-scope="data1">
                            <v-chip :selected="data1.selected" close class="chip--select-multi"
                                    @input="removeMemberships(data1.item.id)">
                              <v-avatar class="primary white--text">
                                {{ data1.item.name.slice(0, 1).toUpperCase() }}
                              </v-avatar>
                              {{ data1.item.name }}
                            </v-chip>
                          </template>
                    ...
                    <v-flex xs12 class="px-0">
                      <v-flex xs12 class="px-0">
                        <v-autocomplete v-model="friends" :disabled="isUpdating" :items="memberGroups"
                                        :rules="[rules.required]" :no-data-text="$t('message.no_message')"
                                        :hint="$t('global.groups_text')" box chips color="purple darken-2"
                                        :label="$t('chamber.member.select_item')" item-text="name" item-value="id"
                                        multiple>
                  ...
                  <v-flex xs12>
                    <v-checkbox color="purple darken-2" :label="$t('global.isSendEmail')"
                                v-model="forms.isSendEmail"></v-checkbox>
                  </v-flex>
                </v-layout>
              </v-form>
            </v-container>
    ...
    methods: {
      ...
      async onSubmit() {
        ...
        if (
          ...
          this.forms.browseRight === 'APPOINT'
        ) {
